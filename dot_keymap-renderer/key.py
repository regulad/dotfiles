...
class Key:
    ...
    def get_font(self, i, size, symbol):
        path = str(Path(__file__).resolve().parent.joinpath('fonts/{}_font.ttf'.format(self.get_full_profile()[0])))
...
def open_base_img(full_profile, res, base_color, color):
    ...
    with Image.open(Path(__file__).resolve().parent.joinpath('images/{0}_{1}{2}.png'.format(*full_profile, base_num))) as img:
