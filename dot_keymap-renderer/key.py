...
class Key:
    ...
    def get_full_profile(self):
        ...
        if props in special_keys:
            ...
            row_profile = "STEP"
        elif (self.width >= 6.0 and self.height == 1.0) or (
            full_profile[-1] == "SPACE"
        ):
            row_profile = "SPACE"
        else:
            row_profile = "BASE"
    ...
    def get_font(self, i, size, symbol):
        path = str(
            Path(__file__)
            .resolve()
            .parent.joinpath("fonts/{}_font.ttf".format(self.get_full_profile()[0]))
        )
        if symbol:
            return ImageFont.truetype(path, size)
        try:
            return ImageFont.truetype(io.BytesIO(self.fonts[i]), size)
        except Exception:
            return ImageFont.truetype(path, size)
    ...
    def get_label_props(self):
        if self.decal:
            ...
            if len(self.labels) and self.label_sizes[0] == 8:
                props["margin_top"] -= 0.05
        else:
            props = {
                "margin_x": 0.22,
                "margin_top": 0.16,
        ...
        center_front, center_row, center_col = [
            digit == "1" for digit in "{0:03b}".format(align)
        ]
        ...
        for i in range(min(len(self.labels), 12)):
            ...
            col = (
                (1 if col < 1 else -1)
                if (center_col and row < 3) or (center_front and row > 2)
                else col
            )
            row = (1 if row < 1 else -1) if (center_row and row < 3) else row
            label_size = self.label_sizes[i] if row != None and row < 3 else 3.0
            props["font_sizes"].append(
                int(0.09 * self.res + 0.03 * self.res * label_size)
            )
    ...
    def get_location(self, key_img):
        ...
        if self.rotation_angle != 0 or self.rotation_x != 0 or self.rotation_y != 0:
            ...
            x2, y2 = x * math.cos(a) - y * math.sin(a), y * math.cos(a) + x * math.sin(
                a
            )

            left, top = -self.width / 2, -self.height / 2
            left2, top2 = left * math.cos(a) - top * math.sin(a), top * math.cos(
                a
            ) + left * math.sin(a)

            x, y = (
                rx + x2 - key_img.width / u / 2 - left2,
                ry + y2 - key_img.height / u / 2 - top2,
            )
        return (
            int(i)
            for i in (x * u, y * u, x * u + key_img.width, y * u + key_img.height)
        )

    def get_model_location(self):
        # get bounding box of model in x/y plane
        x, y, res = (
            min(self.x, self.x + self.x2),
            min(self.y, self.y + self.y2),
            self.model_res,
        )
        ...
        if self.rotation_angle != 0 or self.rotation_x != 0 or self.rotation_y != 0:
            ...
            x, y = rx + x * math.cos(a) - y * math.sin(a), ry + y * math.cos(
                a
            ) + x * math.sin(a)
            width, height = width * math.cos(a) - height * math.sin(
                a
            ), height * math.cos(a) + width * math.sin(a)
    ...
    def get_base_img(self, full_profile):
        if self.flat:
            res, color, row, sizes = (
                ...
                self.color,
                full_profile[1],
                {"ISO": (1.5, 2), "BIGENTER": (2.25, 2)},
            )
            return Image.new(
                "RGBA",
                [int(res * x) for x in sizes.get(row, (1, 1))],
    ...
    def stretch_img(self, base_img, width, height):
        ...
        if height > h:
            ...
            for i in range(1, height - h + 1, 10):
                new_img.paste(
                    middle_part, (0, int(h / 2) + i, new_img.width, int(h / 2) + i + 10)
                )
    ...
    def stretch_model(self, model, width, height):
        if width > 1:
            ...
            for v in model.data.vertices:
                v.co[0] -= (
                    (width - 1) * self.model_res if v.co[0] < -self.model_res / 2 else 0
                )
        elif width < 1:
            ...
            for v in model.data.vertices:
                v.co[0] = (
                    v.co[0]
                    if v.co[0] > -mid
                    else (-mid if v.co[0] > -res + mid else v.co[0] + res - mid * 2)
                )
        ...
        if height > 1:
            ...
            for v in model.data.vertices:
                v.co[1] += (
                    (height - 1) * self.model_res if v.co[1] > self.model_res / 2 else 0
                )
        elif height < 1:
            ...
            for v in model.data.vertices:
                v.co[1] = (
                    v.co[1]
                    if v.co[1] < mid
                    else (mid if v.co[1] < res - mid else v.co[1] - res + mid * 2)
                )
            for p in model.data.polygons:
                p.use_smooth = False
    ...
    def create_key(self):
        ...
        if self.decal:
            return Image.new(
                "RGBA", (int(self.width * self.res + 1), int(self.height * self.res))
            )
            ...
            return self.stretch_img(
                base_img, int(self.width * self.res + 1), int(self.height * self.res)
            )
        else:
            # calculate total width of keycap
            u, x2, y2 = self.res, self.x2, self.y2
            width = (
                max(self.width2 + x2, self.width)
                if x2 >= 0
                else max(self.width - x2, self.width2)
            )
            height = (
                max(self.height2 + y2, self.height)
                if y2 >= 0
                else max(self.height - y2, self.height2)
            )
            # create touch surface
            key_img = Image.new("RGBA", (int(width * u + 1), int(height * u)))
            base_img = self.get_base_img((profile, "BASE"))
            touch_surface = self.stretch_img(
                base_img, int(self.width * u + 1), int(self.height * u)
            )
            ...
            if row_profile == "STEP":
                ...
                if x2 < 0:
                    left_img = (
                        self.get_base_img((profile, row_profile))
                        .copy()
                        .transpose(Image.FLIP_LEFT_RIGHT)
                    )
                    left_step = self.stretch_img(
                        left_img, int(-x2 * u + overlap + 1), int(height * u)
                    )
                ...
                if max(-x2, 0) + self.width < width:
                    ...
                    img_width = int(
                        (width - self.width - max(-x2, 0)) * u + overlap + 1
                    )
                ...
                extra_surface = self.stretch_img(
                    extra_img, int(self.width2 * u + 1), int(self.height2 * u)
                )
    ...
    def create_model(self, scene):
        profile, row_profile = self.get_full_profile()
        if self.str_profile.startswith("DSA"):
            profile = "DSA"
        ...
        if self.decal:
            ...
            if row_profile == "STEP":
                ...
                if x2 < 0:
                    ...
                    for v in left_step.data.vertices:
                        v.co[0] = -model_res * 0.97 - v.co[0]
                ...
                if max(-x2, 0) + self.width < width:
                    ...
                    self.stretch_model(
                        right_step, width - self.width - max(-x2, 0) + 0.333, height
                    )
                    right_step.location = (
                        -(max(-x2, 0) + self.width - 0.333) * model_res,
                        0,
                        0,
                    )
    ...
    def pic_key(self, key_img):
        try:
            ...
            size = (
                width - props["margin_x"] * 2,
                height - props["margin_top"] - props["margin_bottom"],
            )
            with Image.open(requests.get(self.labels[0], stream=True).raw) as label_img:
                ...
                key_img.paste(
                    pic_img, (props["margin_x"], props["margin_top"]), mask=pic_img
                )
            return key_img
        except Exception:
            return key_img
    ...
    def label_key(self, key_img):
        ...
        if len(self.labels) < 1:
            return key_img
        if self.pic:
            return self.pic_key(key_img)

        props = self.get_label_props()
        width, height = int(self.width * self.res), int(self.height * self.res)
        x_offset, y_offset = max(int(-self.x2 * self.res), 0), max(
            int(-self.y2 * self.res), 0
        )
        col2x = [
        ...
        row2y = [
            ...
            lambda h: (height - props["margin_bottom"] + props["margin_top"] - h) / 2
            + y_offset,
        ...
        front_plane = Image.new(
            "RGBA", (width, max(height - props["margin_bottom"] * 2, 1))
        )
        front_draw = ImageDraw.Draw(front_plane)

        for i in range(min(len(self.labels), 12)):
            (row, col), text = props["positions"][i], self.labels[i]
            if not text or row == None:
                continue
            ...
            text = (
                break_text(text, font, width - props["margin_x"] * 2)
                if not self.decal
                else text
            )
            text = (
                text.upper()
                if self.get_full_profile()[0] != "GMK" and not self.decal
                else text
            )
            text_width, text_height = font.getsize_multiline(
                text, spacing=props["line_spacing"]
            )
            ...
            (front_draw if row == 3 else top_draw).multiline_text(
                (col2x[col](text_width), row2y[row](text_height)),
                text,
                font=font,
                fill=color,
        ...
        front_plane = front_plane.resize(
            (width, props["margin_bottom"]), resample=Image.BILINEAR
        )
        key_img.paste(
            front_plane,
            (x_offset, height - props["margin_bottom"] + y_offset),
            mask=front_plane,
        )
        return key_img
    ...
    def render(self, scale, flat):
        ...
        if self.ghost:
            key_img.putalpha(Image.new("L", key_img.size, color=64))
        if not flat:
            key_img = key_img.rotate(
                -self.rotation_angle, resample=Image.BILINEAR, expand=1
            )
        return key_img
...
srgb_profile, lab_profile = ImageCms.createProfile("sRGB"), ImageCms.createProfile(
    "LAB", colorTemp=5000
)
...
def open_base_img(full_profile, res, base_color, color):
    ...
    with Image.open(
        Path(__file__)
        .resolve()
        .parent.joinpath("images/{0}_{1}{2}.png".format(*full_profile, base_num))
    ) as img:
        key_img = img.resize(
            (int(s * res / 200) for s in img.size), resample=Image.BILINEAR
        ).convert("RGBA")
    if full_profile[1] in ("ISO", "BIGENTER"):
        alpha = key_img.split()[-1]
    ...
    key_img = ImageCms.applyTransform(
        Image.merge("LAB", (l, a, b)), lab2rgb_transform
    ).convert("RGBA")
    if full_profile[1] in ("ISO", "BIGENTER"):
        key_img.putalpha(alpha)
    return key_img
