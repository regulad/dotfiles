...
class Keyboard:
    ...
    def render(self):
        ...
        for key in self.keys:
            self.render_key(key, scale, border)
    ...
    def render_key(self, key, scale, border):
        ...
        self.max_size = [
            max(location[2], self.max_size[0]),
            max(location[3], self.max_size[1]),
        ]
        self.expand_keyboard(scale)
        self.keyboard.paste(key_img, (location[0], location[1]), mask=key_img)

    def expand_keyboard(self, scale):
        if all(self.max_size[i] < self.keyboard.size[i] for i in range(2)):
            return
    ...
    def watermark_keyboard(self, text, scale):
        ...
        font = ImageFont.truetype(str(Path(__file__).resolve().parent.joinpath("fonts/SA_font.ttf")), int(36 / scale))
        w, h = font.getsize(text)
        self.max_size = size = [
            max(w, self.max_size[0]),
            (self.max_size[1] + h + margin * 2),
        ]

        # draw watermark bar below image
        draw.rectangle(
            (0, size[1] - h - margin * 2, size[0] + 1, size[1] + 1),
...
def get_fonts(css):
    ...
    def is_str(x):
        return x.type == "string" or x.type == "ident"

    def is_ttf(x):
        return x.type == "url" and ".ttf" in x.value
    ...
    for rule in tinycss2.parse_stylesheet(css):
        ...
        if rule.type == "qualified-rule":
            ...
            for sel in rule.prelude:
                for i in range(12):
                    if sel == "*":
                        fonts[i] = fonts[i] or font
                    if sel.type != "ident":
                        continue
                    ids = ("keylabels", "keylabel", f"keylabel{i}")
                    if sel.lower_value in ids:
                        fonts[i] = font
        ...
        if rule.type == "at-rule" and rule.lower_at_keyword == "font-face":
            ...
            if url:
                ttfs[name] = requests.get(url).content
    return fonts
...
def deserialise(rows):
    ...
    with open(
        str(Path(__file__).resolve().parent.joinpath("fonts/fa2unicode.json"))
    ) as fa, open(
        str(Path(__file__).resolve().parent.joinpath("fonts/kbd-webfont2unicode.json"))
    ) as kb:
        ...
        if isinstance(row, list):
            for key in row:
                if isinstance(key, str):
                    ...
                    if "fa" in key:
                        label_sizes = [
                            float(size) if size > 0 else default_size
                            for size in key["fa"]
                        ]
                        current.label_sizes = label_sizes[:12] + [default_size] * (
                            12 - len(label_sizes)
                        )
                    if "p" in key:
                    ...
                    if "c" in key:
                        color = key["c"].replace(";", "")
                        current.color = (
                            color if color_format.match(color) else current.color
                        )
                    if "t" in key:
                        colors = [
                            line.replace(";", "") for line in key["t"].splitlines()
                        ]
                        default_color = (
                            colors[0]
                            if colors and color_format.match(colors[0])
                            else "#000"
                        )
                        colors = [
                            color if color_format.match(color) else default_color
                            for color in colors
                        ]
                        current.label_colors = colors[:12] + [default_color] * (
                            12 - len(colors)
                        )
                    if "x" in key:
                    ...
                    if "h2" in key:
                        ...
                        current.width2 = (
                            current.width if current.width2 == 0.0 else current.width2
                        )
                    if "l" in key:
                        current.step = key["l"]
                    if "g" in key:
            ...
            if "css" in row:
                try:
                    current.fonts = get_fonts(row["css"])
                except Exception:
                    pass
